...
export function mergeStreamPage(current:SessionTimeline,page:SessionTimeline):SessionTimeline {
  ...
  return {...current,revision:page.revision,spans:current.spans.map((span)=>span.id===target?{...span,stream:[...incoming.stream,...span.stream].filter((item,index,all)=>all.findIndex((candidate)=>candidate.id===item.id)===index),streamBounds:incoming.streamBounds}:span)};
}
...
export function isMundaneShort(span:ProcessSpan,now=Date.now()):boolean{return span.actor==='worker'&&span.state==='completed'&&duration(span,now)<=60_000&&span.stream.length>0&&span.stream.every((item)=>item.authority!=='evidence'&&item.lifecycle!=='failed'&&!['question','narration','agent'].includes(item.kind))}
export function bundleMundaneProcesses(spans:ProcessSpan[],now=Date.now()):TimelineProcess[]{const sorted=[...spans].sort((a,b)=>Date.parse(a.startedAt)-Date.parse(b.startedAt));const out:TimelineProcess[]=[];let pending:ProcessSpan[]=[];const flush=()=>{if(pending.length>=2){const first=pending[0]!,last=pending.at(-1)!;out.push({...first,id:`bundle:${pending.map((item)=>item.id).join('+')}`,label:`${pending.length} short worker processes`,startedAt:first.startedAt,finishedAt:last.finishedAt,stream:pending.flatMap((item)=>item.stream),streamBounds:{complete:pending.every((item)=>item.streamBounds.complete),hasEarlier:pending.some((item)=>item.streamBounds.hasEarlier)},bundle:true,processes:pending})}else out.push(...pending);pending=[]};for(const span of sorted){const previous=pending.at(-1);if(isMundaneShort(span,now)&&(!previous||Date.parse(span.startedAt)-Date.parse(previous.finishedAt??previous.startedAt)<=30_000))pending.push(span);else{flush();if(isMundaneShort(span,now))pending=[span];else out.push(span)}}flush();return out}

...
export function mergeTimelineRefresh(current:SessionTimeline,next:SessionTimeline):SessionTimeline {
  ...
  return {...next,spans:next.spans.map((span)=>{const prior=existing.get(span.id);if(!prior)return span;const incomingIds=new Set(span.stream.map((item)=>item.id));const firstShared=prior.stream.findIndex((item)=>incomingIds.has(item.id));const retainedEarlier=(firstShared<0?prior.stream:prior.stream.slice(0,firstShared)).filter((item)=>!incomingIds.has(item.id));return{...span,stream:[...retainedEarlier,...span.stream],streamBounds:prior.streamBounds}})};
}
...
export interface FeedProcess { process:ProcessSpan; lane:number }
export interface FeedGroup { id:string; start:number; end:number; laneCount:number; processes:FeedProcess[] }
/** Chronology is vertical; only genuine temporal overlap becomes horizontal composition. */
export function buildFeedGroups(spans:ProcessSpan[],now=Date.now()):FeedGroup[]{const sorted=[...spans].sort((a,b)=>Date.parse(a.startedAt)-Date.parse(b.startedAt)||a.id.localeCompare(b.id)),groups:FeedGroup[]=[];for(const process of sorted){const start=Date.parse(process.startedAt),end=process.finishedAt?Date.parse(process.finishedAt):now,last=groups.at(-1);if(!last||start>=last.end){groups.push({id:`feed:${process.id}`,start,end,laneCount:1,processes:[{process,lane:0}]});continue}const laneEnds=Array.from({length:last.laneCount},()=>Number.NEGATIVE_INFINITY);for(const placed of last.processes)laneEnds[placed.lane]=Math.max(laneEnds[placed.lane]!,placed.process.finishedAt?Date.parse(placed.process.finishedAt):now);let lane=laneEnds.findIndex((laneEnd)=>laneEnd<=start);if(lane<0){lane=laneEnds.length;last.laneCount++}last.processes.push({process,lane});last.end=Math.max(last.end,end);last.id=`feed:${last.processes.map((item)=>item.process.id).join(':')}`}return groups}
